...
const game = {
     ...
     */
    start() {
        window.addEventListener('keydown', game.animate);
        let changeButtonText = document.getElementById("button-start");
        changeButtonText.innerText = "Начать";
        let status = game.getElement().classList[1];
        if (status === GAME_STATUS_STARTED | status === GAME_STATUS_STOPPED) {
            board.clear();
            snake.parts = Object.assign([], SNAKE_DEFAULT_PARTS);
    ...
    pause() {
        if (!this.snakeMove) {
            alert("Прежде чем делать паузу, начните движение.");
            return;
        }
        this.setGameStatus(GAME_STATUS_PAUSED);
        let changeButtonText = document.getElementById("button-start");
        changeButtonText.innerText = "Продолжить";
        game.loop();
        /* добавить сюда код */
    },
    ...
    gameOver() {
        let score = document.getElementById("score");
            score.innerText = 0;
        alert("You lose.");
        window.removeEventListener("keydown", game.animate);
    },

    loop() {
        let status = this.getElement().classList[1];
        if (status === GAME_STATUS_STOPPED) {
            if (this.snakeMove) {
    ...
    move(direction) {
        ...
        for (position of snake.parts) {
            if (position.top === nextPosition.top &&
